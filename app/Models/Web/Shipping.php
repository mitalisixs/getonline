...
class Shipping extends Model
{
    ...
    public function addMyAddress($request,$is_default=0)
    {
        ...
        $entry_suburb = isset($request->entry_suburb)?$request->entry_suburb:'';
        $entry_postcode = $request->entry_postcode;
        $entry_city = $request->entry_city;
        $entry_state = isset($request->entry_state)?$request->entry_state:'';
        $entry_country_id = $request->entry_country_id;
        $entry_zone_id = $request->entry_zone_id;
        $entry_gender = isset($request->entry_gender)?$request->entry_gender:'';
        $entry_company = isset($request->entry_company)?$request->entry_company:'';
        $entry_phone = isset($request->entry_phone)?$request->entry_phone:'';
        ...
        if (!empty($customers_id)) {
            ...
            // if($is_default==1){
            //      $this->updateCustomer(auth()->guard('customer')->user()->id, $address_book_id);
            // }
    ...
    public function getShippingAddress($address_id)
    {
        ...
        $addresses = DB::table('user_to_address')
            ...
            ->select(
                ...
                'address_book.entry_phone as delivery_phone',
