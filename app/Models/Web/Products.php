...
///////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if (count($products) > 0) {
            ...
            foreach ($products as $products_data) {
                ...
                if ($default_image_thumb) {
                    $products_data->default_thumb = isset($default_image_thumb->image_path)?$default_image_thumb->image_path:'';
                } else {
    ...
    public function productQuantity($data)
    {
        if (!empty($data['attributes'])) {
            ...
            ->LeftJoin('image_categories', 'products_attribute_images.image', '=', 'image_categories.image_id')
            ->select('image_categories.path as image_path', 'image_categories.image_type')
            ->whereIn('products_attribute_images.product_attribute_id', [$attributeid])
            ->orderBy('sort_order', 'ASC')
            ->get();
            //dd($products_images);
