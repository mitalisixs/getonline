...
class OrdersController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        if (count($result['cart']) == 0) {
            ...
            return view("web.checkout", ['title' => $title, 'final_theme' => $final_theme])
                ->with('result', $result)->with('all_addresses', $all_addresses);
        }
    ...
    public function checkout_shipping_address(Request $request)
    {
        ...
        if(isset($request->shipping_address_id)){
            if($request->shipping_address_id=="add_new_address"){
                $request_data = (array)$request->all();
                $addaddress=  [
                    "entry_firstname"=>$request_data["firstname"],
                    "entry_lastname"=>$request_data["lastname"],
                    "entry_street_address"=>$request_data["street"],
                    "entry_country_id"=>$request_data["countries_id"],
                    "entry_zone_id"=>$request_data["zone_id"],
                    "entry_city"=>$request_data["city"],
                    "entry_postcode"=>$request_data["postcode"],
                    "entry_suburb"=>"",
                    "entry_phone"=>$request_data["delivery_phone"]
                ];
                $addaddress = (object) $addaddress;
                $address_id = $this->shipping->addMyAddress($addaddress);
