<?php
   ...
   // dd($categories);
    foreach($categories as $parents){
     ...
     if(isset($parents->childs)){
      ...
      $option .= '
       <a class=" main-manu"'. $hasChild .'> <img class="img-fuild" src="'.asset($parents->path).'"> '.ucwords(strtolower($parents->categories_name)).'</a>';
  ...
  foreach($childs as $key => $child){
    ...
    $contents.= '<li class="list-item">
        <a class="list-link" href='.url('shop?category=').$child->slug.' > '.$dash.'
            <i class="fas fa-angle-right"></i>'.ucwords(strtolower($child->categories_name)).'
        </a>
      </li>';
 ...
 ?>
<?php /**PATH /var/www/vhosts/centrifugalproducts.com/httpdocs/resources/views/web/common/shopCategories.blade.php ENDPATH**/ ?>